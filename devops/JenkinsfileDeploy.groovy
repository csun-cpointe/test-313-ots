            ...
            try {
                dir(projectDirectory) {
                    slackSend color: "warning",
                            message: "test-313 authenticating"
                    jenkinsSteps.argocdAuthenticate(argocdUrl, '${argocdPassword}')
                }
            } catch (err) {
                slackSend color: "danger",
                        message: "test-313 failed to authenticate properly"
                throw err
            }
        }
    }
        ...
        try {
            dir(projectDirectory) {
                ...
                slackSend color: "good",
                        message: "test-313 deployed successfully"
            }
        } catch (err) {
            slackSend color: "danger",
                    message: "test-313 failed to deploy properly"
            throw err
        }
    }
}
...
stage("Teardown") {
    node('master') {
        try {
            dir(projectDirectory) {
                slackSend color: "warning",
                        message: "test-313 shutting down"
                jenkinsSteps.argocdTerminate(argocdAppName)
            }
        } catch (err) {
            slackSend color: "danger",
                    message: "test-313 failed to shut down properly"
            throw err
        }
    }
}
